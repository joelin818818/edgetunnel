...
///////////////////////////////////////////////////////文件头可配置参数（替代原 KV config.json 持久化）///////////////////////////////////////////////
// 说明：已取消 KV 持久化，所有配置直接写在下面 USER_CONFIG 中。修改后需重新部署 Worker。
// 仅填写需要覆盖的字段即可，未填写的字段将使用脚本内置默认值。
const USER_CONFIG = {
	// ===== 基础节点参数 =====
	// HOST: '你的域名.com',               // 节点显示域名（留空则用请求域名）
	// UUID: '生成的UUID',                 // 用户 UUID（留空则用变量 UUID）
	// PATH: '/自定义路径',                // 节点路径（留空则用变量 PATH 或 '/'）
	协议类型: 'vless',                    // vless / ss
	传输协议: 'ws',                       // ws / grpc
	gRPC模式: 'gun',                      // gun / multi
	跳过证书验证: false,
	启用0RTT: true,
	TLS分片: true,
	随机路径: true,
	...
	// SS: { 加密方式: 'aes-128-gcm', TLS: false },

	// ===== 优选订阅生成（仅保留本地随机IP库，已取消 ADD.txt 与远程优选订阅生成器）=====
	优选订阅生成: {
		local: true,
		SUBNAME: 'EdgeTunnel',
		订阅名: 'CF-EdgeTunnel',
		更新时间: '副标题',
		随机数量: 200,
		SUB: '',                          // 原远程优选订阅生成器（已取消，留空）
		本地IP库: {
			随机IP: true,                 // 固定使用本地随机IP库
			随机数量: 200,
			指定端口: '443',
			区域限制: 'HK,KR,JP,SG,US',
			自动优选: false,
	...
	// ===== 订阅转换配置（subconverter）=====
	订阅转换配置: {
		SUBAPI: 'subapi.herokuapp.com',
		SUBCONFIG: 'https://raw.githubusercontent.com/cmliu/edgetunnel/main/sub.ini',
		CLASH: false,
		SURGE: false,
		LOON: false,
		QUANX: false,
	...
	// ===== 反代代理链（SOCKS5/HTTP/TURN/SSTP）=====
	反代: {
		SOCKS5: { 启用: false, 账号: '', 全局: false },
		路径模板: { 默认: 'proxyip=proxyip.example.com' }
	},

	// ===== 已取消的功能（保留字段仅为兼容，不再生效）=====
...
export default {
	async fetch(request, env, ctx) {
		...
		if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
		...
		} else if (访问路径 === 'login') {//处理登录页面和登录请求（已取消配置保存/重置/日志/KV，仅保留只读查看）
		...
		} else if (访问路径 === 'admin' || 访问路径.startsWith('admin/')) {//验证cookie后响应管理页面（已取消 KV 持久化、TG、CF、ADD、代理检测、日志功能）
			const cookies = request.headers.get('Cookie') || '';
			const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
			...
			if (访问路径 === 'admin/config.json') {// 处理 admin/config.json 请求，返回当前配置（只读，来自文件头常量）
				return new Response(JSON.stringify(config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json' } });
			} else if (区分大小写访问路径 === 'admin/ADD.txt') {// 处理 admin/ADD.txt 请求，返回本地随机优选IP库
				const 本地优选IP = (await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口))[1];
				return new Response(本地优选IP, { status: 200, headers: { 'Content-Type': 'text/plain;charset=utf-8', 'asn': request.cf.asn } });
			} else if (访问路径 === 'admin/cf.json') {// CF 请求信息（只读）
				return new Response(JSON.stringify(request.cf, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
			}

			// 注意：原版的配置保存/重置(KV)、TG 通知、CF 用量、ADD 自定义IP、代理连通性检测、日志均已移除
			return fetch(Pages静态页面 + '/admin' + url.search);
				...
				if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
					...
					// 已取消 CF 用量查询功能，不再添加 Subscription-Userinfo 用量头
					const isSubConverterRequest = url.searchParams.has('b64') || url.searchParams.has('base64') || request.headers.get('subconverter-request') || request.headers.get('subconverter-version') || ua.includes('subconverter') || ua.includes(('CF-Workers-SUB').toLowerCase()) || 作为优选订阅生成器;
						...
						if (config_JSON.优选订阅生成.local) { // 本地随机IP库生成订阅（已取消 ADD.txt 与远程优选订阅生成器）
							const 完整优选列表 = await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口);
							const 完整优选IP数组 = 完整优选列表[0];
							完整优选IP = 完整优选IP.concat(完整优选IP数组);
						}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		优选订阅生成: {
			...
			本地IP库: {
				随机IP: true, // 启用本地随机IP库（已取消 KV 内的 ADD.txt，固定使用随机IP库）
				随机数量: 16,
				指定端口: -1,
			},
			SUB: null,
	...
	try {
		// 取消 KV 持久化：将文件头 USER_CONFIG 深度合并到内置默认配置（替代原 config.json）
		const 深度合并 = (目标, 源) => {
			for (const key in 源) {
				if (源[key] && typeof 源[key] === 'object' && !Array.isArray(源[key])) {
					目标[key] = 深度合并(目标[key] || {}, 源[key]);
				} else if (源[key] !== undefined) {
					目标[key] = 源[key];
				}
			}
			return 目标;
		};
		config_JSON = 深度合并(JSON.parse(JSON.stringify(默认配置JSON)), USER_CONFIG);
	} catch (error) {
	...
	// 已取消 Telegram 机器人通知（tg.json）功能，配置内 TG 始终为禁用
	config_JSON.TG = { 启用: false, BotToken: null, ChatID: null };
	// 已取消 Cloudflare API 用量查询（cf.json）功能，CF 始终为空
	config_JSON.CF = { Email: null, GlobalAPIKey: null, AccountID: null, APIToken: null, UsageAPI: null, Usage: { success: false, pages: 0, workers: 0, total: 0, max: 100000 } };
